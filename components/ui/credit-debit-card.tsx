...
// --- PROPS INTERFACE ---
// Defines the types for props that the component accepts.
...
const FlippableCreditCard = React.forwardRef<HTMLDivElement, FlippableCreditCardProps>(
  ({ className, cardholderName, cardNumber, expiryDate, cvv, ...props }, ref) => {
    return (
      // The main container uses `group` to control flip effect on hover.
      // `perspective` is used to create 3D effect.
      <div
        className={cn("group h-40 w-64 [perspective:1000px]", className)}
        ref={ref}
        {...props}
      >
        {/* The inner container handles the continuous rotation animation */}
        <div className="relative h-full w-full rounded-xl shadow-xl transition-transform duration-300 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] animate-spin-continuous">
          
          {/* --- CARD FRONT --- */}
          <div className="absolute h-full w-full rounded-xl bg-black text-white [backface-visibility:hidden]">
            <div className="relative flex h-full flex-col justify-between p-4">
              ...
              <div className="flex items-end justify-between">
                <div className="text-left">
                  <p className="text-xs font-semibold uppercase opacity-90 text-gray-300">Card Holder</p>
                  <p className="font-mono text-sm font-medium text-white">{cardholderName}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-semibold uppercase opacity-90 text-gray-300">Expires</p>
                  <p className="font-mono text-sm font-medium text-white">{expiryDate}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* --- CARD BACK --- */}
          <div className="absolute h-full w-full rounded-xl bg-black text-white [backface-visibility:hidden] [transform:rotateY(180deg)]">
            <div className="flex h-full flex-col">
              {/* Magnetic Strip */}
              <div className="mt-6 h-10 w-full bg-gray-800" />
              {/* CVV Section */}
              <div className="mx-4 mt-4 flex justify-end">
                <div className="flex h-8 w-full items-center justify-end rounded-md bg-white pr-4">
                  <p className="font-mono text-sm text-black">{cvv}</p>
                </div>
              </div>
              <p className="self-end pr-4 text-xs font-semibold uppercase opacity-90 text-gray-300">CVV</p>
